...
export function Cta({ enableScrollAnimation = true }: { enableScrollAnimation?: boolean }) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Left content appears at 20% scroll progress
      if (scrollProgress > 0.2 && !animationTriggered.leftContent && leftContentRef.current) {
        animationTriggered.leftContent = true;
      ...
      // Right content appears at 30% scroll progress
      if (scrollProgress > 0.3 && !animationTriggered.rightContent && rightContentRef.current) {
        animationTriggered.rightContent = true;
  ...
  return (
    <>
      ...
      <LocalBusinessSchema
        name="Bijou Group"
        url="https://bijougroup.co.uk/cta"
        address={{
        ...
        logo="/logo.svg"
      />
      <section ref={sectionRef} className="w-full max-w-7xl mx-auto mt-24 mb-16 px-2 sm:px-4 flex flex-col md:flex-row items-center justify-between gap-8 bg-transparent border-none shadow-none">
        {/* Left: Title and Buttons */}
        <div ref={leftContentRef} className={`flex-1 flex flex-col items-start justify-center min-w-0 w-full max-w-full ${enableScrollAnimation ? 'opacity-0' : ''}`}>
        ...
        {/* Right: Description */}
        <div ref={rightContentRef} className={`flex-1 flex flex-col items-center justify-center max-w-xl text-[#12443A] text-base xs:text-lg sm:text-[18px] font-space text-center w-full ${enableScrollAnimation ? 'opacity-0' : ''}`}>
